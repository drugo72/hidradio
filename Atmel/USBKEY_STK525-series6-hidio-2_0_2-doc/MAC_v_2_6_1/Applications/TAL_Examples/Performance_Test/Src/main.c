 ...
 *
 * $Id: main.c 25503 2011-02-10 10:30:03Z yogesh.bellan $
 *
...
void tal_rx_frame_cb(frame_info_t *frame)
{
    ...
    if (op_mode == PROMISCUOUS_OP_MODE)
    {
        ...
        {
            sprintf(ascii, "%.2" PRIX8 " ", frame->mpdu[i]);
            printf(ascii);
        }
        printf("\r\n");
    }
...
static void configure_pibs(void)
{
    ...
    temp_value_16 = CPU_ENDIAN_TO_LE16(OWN_SHORT_ADDR);
    tal_pib_set(macShortAddress, (pib_value_t *)&temp_value_16);

    temp_value_16 = CPU_ENDIAN_TO_LE16(SRC_PAN_ID);
...
static void get_sensor_data(void)
{
    ...
    if (bat_mon == SUPPLY_VOLTAGE_ABOVE_UPPER_LIMIT)
    {
        printf("\r\nBattery monitor: Voltage above max. supply voltage");
    }
    else
    {
        printf("\r\nBattery monitor: U = %" PRIu16 ".%03" PRIu16 " V\r\n", bat_mon/1000, bat_mon%1000);
    }
