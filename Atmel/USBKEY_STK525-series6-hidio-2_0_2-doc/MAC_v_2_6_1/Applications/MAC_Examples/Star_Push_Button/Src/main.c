 ...
 * @file main.c
 *
 * @brief  MAC Example - Star Push Button Network
 *
 * This is the source code of a simple MAC example. It implements the
 * firmware for all nodes of a network with star topology.
 * The associated devices will go into complete system sleep
 * (MCU and Transceiver) wake up only once the button is pressed.
 *
 * $Id: main.c 24922 2011-01-17 09:22:31Z yogesh.bellan $
 *
...
static void app_task(void)
{
    switch (node_status)
    {
        case IDLE:
            {
                button_state_t button = pal_button_read(BUTTON_0);
