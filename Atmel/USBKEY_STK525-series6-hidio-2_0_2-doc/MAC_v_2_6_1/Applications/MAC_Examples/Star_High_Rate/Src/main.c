 ...
 * @file main.c
 *
 * @brief  MAC Example - Star High Rate
 *
 * This is the source code of a simple MAC example. It implements the
 * firmware for all nodes of a network with star topology.
 * The nodes exchange data frames using high rates (i.e. 2Mbit/s).
 *
 * For more information how to change the data rate see the AVR2025_User_Guide.pdf.
 *
 * For information about the actual values of the channel pages see
 * the corresponding file tal_pib.c and the handling of the TAL
 * PIB attribute phyCurrentPage (see variable tal_pib_CurrentPage).
 *
 * $Id: main.c 25406 2011-02-07 13:16:35Z yogesh.bellan $
 *
...
/** This variable stores the current type of the node. */
static app_type_t app_type = APP_IDLE;
...
#ifdef SIO_HUB
            // No network could be found; therefore start new network.
            printf("No existing network found\n");
#endif
            /*
             * Set the short address of this node.
             * Use: bool wpan_mlme_set_req(uint8_t PIBAttribute,
...
void usr_mlme_set_conf(uint8_t status, uint8_t PIBAttribute)
{
    ...
    else if ((status == MAC_SUCCESS) && (PIBAttribute == phyCurrentPage))
    {
        /*
         * The channel page for high data rates was set successfully.
         * Do nothing for the Coordinator.
         * The device starts the timer to initiate high data rate frame transmission.
         */
        if (app_type == APP_DEVICE)
        {
#ifdef SIO_HUB
            printf("Device switched to High Rate\n");
#endif
        ...
        {
#ifdef SIO_HUB
            printf("Coordinator switched to High Rate\n");
#endif
        }
                              ...
                              uint8_t status)
{
    if (status == MAC_SUCCESS)
    {
         ...
         */
        /*
         * Now switch to high data rate by setting the appropriate channel page.
         * In this example the 2Mbit/s mode is used by setting the channel page 17.
         *
         * For information about the actual values of the channel pages see
         * the corresponding file tal_pib.c and the handling of the TAL
         * PIB attribute phyCurrentPage.
         */
         uint8_t channel_page_17_2mbit = 17;

         wpan_mlme_set_req(phyCurrentPage, &channel_page_17_2mbit);
    }
