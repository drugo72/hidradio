 ...
 *
 * $Id: pal_board.c 23083 2010-08-23 14:45:09Z sschneid $
 *
...
void pal_button_init(void)
{
    ...
    BUTTON_PORT |= (1 << BUTTON_PIN_2);
    BUTTON_PORT |= (1 << BUTTON_PIN_3);
    BUTTON_PORT_DIR &= ~(1 << BUTTON_PIN_0);
    BUTTON_PORT_DIR &= ~(1 << BUTTON_PIN_1);
    BUTTON_PORT_DIR &= ~(1 << BUTTON_PIN_2);
    BUTTON_PORT_DIR &= ~(1 << BUTTON_PIN_3);
}
...
uint16_t pal_generate_rand_seed(void)
{
    ...
    /* Start 2nd conversion. */
    while ((ADCSRA & (1 << ADIF)) == 0);
    pal_timer_delay(100);
    cur_random_seed = (cur_random_seed << 8) | ADCL;
