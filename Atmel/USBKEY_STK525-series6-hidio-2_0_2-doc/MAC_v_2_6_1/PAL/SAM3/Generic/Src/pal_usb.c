 ...
 *
 * This file implements SAM3 USB related functions
 * and reception functions.
 *
 * $Id: pal_usb.c 24759 2011-01-06 13:50:26Z mahendran.p $
 *
...
static uint8_t usb_0_tx_temp_buf[USB_TX_BUF_SIZE];
static uint8_t usb_0_rx_temp_buf[USB_RX_BUF_SIZE];

/* USB communication buffer */
static usb_communication_buffer_t usb_0_buffer;

/* Flag to indicate whether the transmission in progress or not */
volatile bool block_tx;

/* VBUS configuration based on the PIN USB VBUS*/
#if defined(PIN_USB_VBUS)
#define VBUS_CONFIGURE()  VBus_Configure()
static const pin_t pinVbus = PIN_USB_VBUS;
#else
#define VBUS_CONFIGURE()    USBD_Connect()
#endif

/* === Prototypes =========================================================*/
void usb0_tx_complete_handler(  void *pArg ,
                                unsigned char   status,
                                unsigned int    transferred,
                                unsigned int    remaining);

void usb0_rx_complete_handler(  void *pArg ,
                                unsigned char status,
...
/**
 * @brief Configures USB clock
 */
void ConfigureUsbClock(void)
{
...
#if defined(PIN_USB_VBUS)
/**
 * @brief Handles VBUS interrupt
 *
 * @param pPin-pin descriptor of the interrupt
 */
static void ISR_Vbus(const pin_t *pPin)
{
    /* Check current level on VBus */
    if (pal_pio_get(&pinVbus))
    {
        USBD_Connect();
    }
    else
...
/**
 * @brief Configures the VBus pin to trigger an interrupt when the
 * level on that pin changes
 */
static void VBus_Configure( void )
{
    /*Configure PIO for usb */
    pal_pio_configure(&pinVbus, PIO_LISTSIZE(pinVbus));
    pal_pio_configure_interrupt(&pinVbus, ISR_Vbus);
    pal_pio_enable_interrupt(&pinVbus);
...
void sio_usb_0_init(void)
{
    ...
    /* Set the flag to false during initialization */
    block_tx = false;

    /* Disconnect to if USB stick has been powered before*/
    USBD_Disconnect();
    ...
    /* Wait until the state is changed to configured state*/
    while (USBD_GetState() < USBD_STATE_CONFIGURED);

    /* USB serial driver read is called to give the pointer to the data buffer
...
uint8_t sio_usb_0_rx(uint8_t *data, uint8_t max_length)
{
    ...
    /* Read the buffer by calling USB driver read */
    if(size > 60)
    {
