 ...
 *
 * $Id: pal_timer.c 25376 2011-02-04 16:15:21Z pawan.jadia $
 *
...
void timer_init(void)
{
    ...
    /*
     * In case RC oscillator calibration is enabled for Mega-RF devices,
     * the 32kHz oscillator needs to be stable. This takes around half
     * a second, before this can be used safely.
     * Therefore a blocking delay of 500ms is included, in order to make sure
     * a potential RC calibration will work properly.
     */
...
/**
 * @brief Adds the sleep time to system time
 *
 * This function adds the sleep time (= watchdog interrupt time) to symbol counter.
 *
 */
void add_sleeptime_to_systime(void)
{
    uint32_t current_time = 0;
    /* Check whether Symbol counter busy flag SCBSY is not set.*/
    while ((SCSR & _BV(SCBSY)) != 0)
    {
        ;
    }
    current_time = SC_READ32(SCCNT);
    current_time += 62500;/* Mac symbols, 1s */
    SC_WRITE32(SCCNT, current_time);;
}
#endif
