 ...
 *
 * $Id: pal_board.c 25242 2011-01-28 08:09:11Z viswanadham.kotla $
 *
...
void pal_led(led_id_t led_no, led_action_t led_setting)
{
    ...
    switch (led_setting)
    {
        case LED_ON: LED_PORT.OUTSET = _BV(pin); break;

        case LED_OFF: LED_PORT.OUTCLR = _BV(pin); break;
...
/**
 * @brief pal_configure_port_interrupt
...
void pal_configure_port_interrupt(void)
{
      ...
      /* Multi-pin Configuration Mask configure for pin3 PB3*/
      PORTCFG.MPCMASK = _BV(BUTTON_PIN_0);

      /*Configure PORT B pin3 control reg- pull up & level sensing*/
      BUTTON_PORT.PIN3CTRL = (uint8_t)PORT_OPC_PULLUP_gc | PORT_ISC_LEVEL_gc;

      /*Configure INTCTRL register for medium level */
...
/**
 * @brief ISR for external interrupt at pin PB3
 */
ISR(PORTB_INT0_vect)
{
      /*disable Interrupt0 for PB3*/
      BUTTON_PORT.INT0MASK &= ~( _BV(BUTTON_PIN_0));
 ...
 * Configure the port interrupt & prepare the system for sleep
...
void pal_sleep_mode(uint8_t sleep_mode)
{
  ...
  /*confiugre the buuton port pin as port ineterrupt */
  pal_configure_port_interrupt();

  /*Prepare the CPU for teh sleep - power down mode */
  CONFIGURE_SLEEP(SLEEP_SMODE_PDOWN_gc);

  /* Enable interrupts before sleeping, to awake later*/
  ENABLE_GLOBAL_IRQ();

   /* enter sleep mode. Port inetrrupt will awake the device */
  CPU_SLEEP();
