 ...
 *
 * $Id: mac_data_req.c 24798 2011-01-10 06:35:43Z yogesh.bellan $
 *
                               ...
                               uint16_t expl_dest_pan_id)
{
    ...
    *frame_ptr = DATAREQUEST;
    ...
    if ((BROADCAST == tal_pib_ShortAddress) ||
        (CCPU_ENDIAN_TO_LE16(MAC_NO_SHORT_ADDR_VALUE) == tal_pib_ShortAddress) ||
        force_own_long_addr)
    {
        frame_ptr -= 8;
    ...
    else
    {
        ...
        /* Build the Source address. */
        convert_16_bit_to_byte_array(tal_pib_ShortAddress, frame_ptr);
    ...
    else
    {
        ...
        if (CCPU_ENDIAN_TO_LE16(MAC_NO_SHORT_ADDR_VALUE) != mac_pib_macCoordShortAddress)
        {
            /*
            ...
            frame_ptr -= 2;
            convert_16_bit_to_byte_array(mac_pib_macCoordShortAddress, frame_ptr);
        }
        else
        {
            /*
...
static buffer_t *build_null_data_frame(void)
{
    ...
    /* No Source addressing information. */
