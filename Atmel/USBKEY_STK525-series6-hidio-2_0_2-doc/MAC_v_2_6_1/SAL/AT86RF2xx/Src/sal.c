 ...
 * implemented in an Atmel's radio transceiver AT86RF2xx.
 *
 * $Id: sal.c 24495 2010-12-13 12:29:11Z sschneid $
 *
 */
/**
 * @author
                   ...
                   uint8_t dir)
{
    if (key != NULL)
    {
        ...
        pal_trx_sram_write((SRAM_BASE_ADDR + RG_AES_CTRL), aes_buf, AES_BLOCKSIZE + 1);
    }
        ...
        case AES_DIR_DECRYPT:
            if (last_dir != AES_DIR_DECRYPT)
            {
                ...
                if (!dec_initialized)
                {
                    ...
                    /* Set to key mode. */
                    aes_buf[0] = SR_MASK(SR_AES_MODE, AES_MODE_KEY);
                    pal_trx_sram_write((SRAM_BASE_ADDR + RG_AES_CTRL), aes_buf, 1);

                    /* Read the key. */
                    pal_trx_sram_read((SRAM_BASE_ADDR + RG_AES_STATE_KEY_0), dec_key, AES_KEYSIZE);
                }
...
void sal_aes_wrrd(uint8_t *idata, uint8_t *odata)
{
    ...
    if (setup_flag)
    {
        pal_trx_aes_wrrd((SRAM_BASE_ADDR + RG_AES_CTRL), aes_buf, AES_BLOCKSIZE + 2);

        setup_flag = false;
    }
    else
    {
        pal_trx_aes_wrrd((SRAM_BASE_ADDR + RG_AES_STATE_KEY_0), aes_buf + 1, AES_BLOCKSIZE + 1);
    }
...
{
    pal_trx_sram_read((SRAM_BASE_ADDR + RG_AES_STATE_KEY_0), data, AES_BLOCKSIZE);
}
